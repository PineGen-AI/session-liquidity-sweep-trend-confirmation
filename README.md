...
A Pine Script v6 TradingView strategy that combines session-based liquidity sweep detection, EMA trend confirmation, ATR stop-loss, risk-reward take-profit, and intraday breakout trading logic.
...
Trades are only considered during a defined session (default 9:30 - 11:00). This helps avoid low-volume periods that can lead to false signals.

### Trend Confirmation

The strategy uses a 50-period EMA to identify the market trend. Long trades are only taken in an uptrend, and short trades in a downtrend.
...
Default position size is 5% of equity per trade, making it suitable for risk-conscious trading.
